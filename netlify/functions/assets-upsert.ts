...
export const handler: Handler = async (event) => {
  ...
  const payload: any = { household_id: session.household_id, name, category: category || null, purchase_price: purchase_price ?? null };
  if (room_id) payload.room_id = room_id;

  if (id) {
    const { error } = await supabase.from('assets').update(payload).eq('id', id).eq('household_id', session.household_id);
